...
def train():
    ...
    model = torch.nn.DataParallel(model, device_ids=devicess,output_device=[1]) #数据并行
    #DataParallel 会自动拆分数据，并将作业订单发送到多个GPU上的多个模型。 在每个模型完成它们的工作之后，DataParallel 在将结果返回给你之前收集和合并结果。
    optimizer = torch.optim.Adam(model.parameters(), lr=args.init_lr)

    # scheduler = ReduceLROnPlateau(optimizer, 'min',factor=0.5, patience=20, verbose=True)
    scheduler = StepLR(optimizer, step_size=30, gamma=0.8) #学习率调整策略
...
def test():
    ...
    for i,subj in enumerate(test_dataset.subjects):
        ...
        if (hp.in_class == 1) and (hp.out_class == 1) :
            ...
            output_image.save(os.path.join(output_dir_test,str(i)+"_result_int.nii.gz"))
        else:
